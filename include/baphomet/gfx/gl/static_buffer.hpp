...
public:
  StaticBuffer(GLsizeiptr size, BufTarget target, BufUsage usage);

  StaticBuffer(const std::vector<T> &data, BufTarget target, BufUsage usage);
...
template<typename T>
StaticBuffer<T>::StaticBuffer(GLsizeiptr size, BufTarget target, BufUsage usage) : BufferBase() {
  target_ = target;
  usage_ = usage;
...
template<typename T>
StaticBuffer<T>::StaticBuffer(const std::vector<T> &data, BufTarget target, BufUsage usage) : BufferBase() {
  target_ = target;
  usage_ = usage;
...
void StaticBuffer<T>::resize(GLsizeiptr new_size, BufTarget target, BufUsage usage) {
  bind(target);
  glBufferData(unwrap(target), sizeof(T) * new_size, nullptr, unwrap(usage));
  unbind(target);
...
void StaticBuffer<T>::write(const std::vector<T> &new_data, BufTarget target, BufUsage usage) {
  bind(target);
  glBufferData(unwrap(target), sizeof(T) * new_data.size(), &new_data[0], unwrap(usage));
  unbind(target);
...
void StaticBuffer<T>::sub_data(const std::vector<T> &new_data, GLintptr offset, BufTarget target) {
  bind(target);
  glBufferSubData(unwrap(target), sizeof(T) * offset, sizeof(T) * new_data.size(), &new_data[0]);
  unbind(target);
...
std::vector<T> StaticBuffer<T>::read(GLintptr offset, GLsizeiptr length, BufTarget target) {
  ...
  auto glptr = glMapBufferRange(unwrap(target), sizeof(T) * offset, sizeof(T) * length, GL_MAP_READ_BIT);
  if (!glptr)
